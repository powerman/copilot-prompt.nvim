...
function M.Gpt54Prompt_render(opts)
    ...
    do
        ...
        table.insert(lines, '')
        table.insert(lines, 'You MUST adhere to the following criteria when solving queries:')
        table.insert(
            lines,
            '- Working on the repo(s) in the current environment is allowed, even if they are proprietary.'
        )
        table.insert(lines, '- Analyzing code for vulnerabilities is allowed.')
        table.insert(lines, '- Showing user code and tool call details is allowed.')
        ...
        table.insert(lines, '- Avoid unneeded complexity in your solution.')
        table.insert(
            lines,
            '- Do not attempt to fix unrelated bugs or broken tests. It is not your responsibility to fix them. (You may mention them to the user in your final message though.)'
        )
        table.insert(lines, '- Update documentation as necessary.')
        table.insert(
            lines,
        ...
        table.insert(parts, tag('task_execution', table.concat(lines, '\n')))
    end
