...
--- Ported from openai/gpt54Prompt.tsx (originally hiddenModelJPrompt).
...
--- Gpt54Prompt — GPT-5.4 system prompt.
...
function M.Gpt54Prompt_render(opts)
    ...
    do
        ...
        if tools.ApplyPatch then
            table.insert(
                ...
                '- Always use '
                    .. tn(tools, 'ApplyPatch')
                    .. " for manual code edits. Do not use cat or any other commands when creating or editing files. Formatting commands or bulk edits don't need to be done with "
                    .. tn(tools, 'ApplyPatch')
                    .. '.'
            )
        end
        ...
        table.insert(lines, '- Do not amend a commit unless explicitly requested to do so.')
        table.insert(
            lines,
        ...
        table.insert(parts, tag('editing_constraints', table.concat(lines, '\n')))
    end
    ...
    -- special_formatting
    table.insert(parts, tag('special_formatting', dai.MathIntegrationRules_render(opts)))
    ...
    -- frontend_tasks
    do
        -- Simplified: no VS Code specific React patterns reference
        local lines = {}
        table.insert(
            lines,
        ...
        table.insert(lines, '- Ensure the page loads properly on both desktop and mobile')
        table.insert(
            lines,
        ...
        table.insert(parts, tag('frontend_tasks', table.concat(lines, '\n')))
    end
    ...
    table.insert(
        ...
        tag(
            'working_with_the_user',
            table.concat({
                'You produce plain text that will later be styled by the program you run in. Formatting should make results easy to scan, but not feel mechanical. Use judgment to decide how much structure adds value. Follow the formatting rules exactly.',
            }, '\n')
        )
    )
    ...
    do
        local lines = {}
        table.insert(lines, '- You may format with GitHub-flavored Markdown.')
        table.insert(
            lines,
        ...
        -- Simplified file references: no markdown links (VS Code specific), use backticks
        local linkText = fileLinkification.render()
        table.insert(lines, linkText)
        table.insert(lines, "- Don't use emojis or em dash unless explicitly instructed.")
        table.insert(parts, tag('formatting_rules', table.concat(lines, '\n')))
    end
    ...
    do
        ...
        table.insert(lines, 'Requirements for your final answer:')
        table.insert(lines, '- Prefer short paragraphs by default.')
        table.insert(
            lines,
        ...
        table.insert(lines, '- Never use nested bullets. Keep lists flat (single level).')
        table.insert(parts, tag('final_answer_instructions', table.concat(lines, '\n')))
    end
    ...
    table.insert(
        ...
        tag(
            ...
            table.concat({
                ...
                "- When exploring, provide user updates as you go, explaining what context you are gathering and what you've learned.",
                ...
                '- As you are thinking, you very frequently provide updates even if not taking any actions, informing the user of your progress.',
            }, '\n')
        )
    )
    ...
    do
        ...
        if tools.ApplyPatch then
            table.insert(
                execLines,
                '- Use the ' .. tn(tools, 'ApplyPatch') .. ' tool to edit files.'
            )
        end
        table.insert(execLines, '')
        ...
        table.insert(
            execLines,
            "- Do not waste tokens by re-reading files after calling a patch tool on them. The tool call will fail if it didn't work."
        )
        table.insert(
            execLines,
