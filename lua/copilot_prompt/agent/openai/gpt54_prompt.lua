--- OpenAI GPT-5.4 model-specific prompts.
--- Ported from gpt54Prompt.tsx.
...
function M.Gpt54Prompt_render(opts)
    ...
    do
        ...
        table.insert(parts, tag('general', table.concat(generalLines, '\n')))
    end
    ...
    -- special_formatting (MathIntegrationRules)
    local mathRules = dai.MathIntegrationRules_render(opts)
    if mathRules ~= '' then
        table.insert(parts, tag('special_formatting', mathRules))
    end

    -- ApplyPatch instructions (conditional)
    ...
    table.insert(
        ...
        tag(
            ...
            table.concat({
                ...
                '- File References: When referencing files in your response follow the rules in the fileLinkification section below.',
    ...
    -- toolUseInstructions (conditional on ExecutionSubagent)
    if tools.ExecutionSubagent then
        table.insert(
            parts,
            tag(
