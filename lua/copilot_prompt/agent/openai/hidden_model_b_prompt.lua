--- Hidden Model B agent prompt.
--- Ported from openai/hiddenModelBPrompt.tsx.
--- This is a more detailed version of the GPT-5.1 prompt with additional
--- sections for design constraints, long context handling, uncertainty
--- handling, and high-risk self-checking.
...
--- HiddenModelBPrompt — Hidden Model B system prompt.
---@param opts Copilot.Options
---@return string
function M.HiddenModelBPrompt_render(opts)
    ...
    -- toolUseInstructions: Don't call ExecutionSubagent multiple times in parallel
    if tools.ExecutionSubagent then
        table.insert(
            parts,
            tag(
    ...
    -- special_formatting: backtick formatting, MathIntegrationRules
    do
        local fmtLines = {}
        table.insert(
            fmtLines,
    ...
    -- McpToolInstructions skipped (VS Code specific)

    if tools.ApplyPatch then
        table.insert(parts, dai.ApplyPatchInstructions_render(opts, tools))
    end

    -- design_and_scope_constraints (hiddenModelB-specific)
    table.insert(
        parts,
        tag(
    ...
    -- long_context_handling (hiddenModelB-specific)
    table.insert(
        parts,
        tag(
    ...
    -- uncertainty_and_ambiguity (hiddenModelB-specific)
    table.insert(
        parts,
        tag(
    ...
    -- high_risk_self_check (hiddenModelB-specific)
    table.insert(
        parts,
        tag(
    ...
    do
        ...
        table.insert(
            fmtLines,
            "The user is working on the same computer as you, and has access to your work. As such there's never a need to show the contents of files you have already written unless the user explicitly asks for them. Similarly, if you've created or modified files using `"
                .. tn(tools, 'ApplyPatch')
        ...
        table.insert(fmtLines, '- For complex multi-step or multi-file tasks:')
        table.insert(fmtLines, '- 1 short overview paragraph')
        table.insert(
            fmtLines,
...
--- HiddenModelBReminderInstructions.
---@param opts Copilot.Options
---@return string
function M.HiddenModelBReminderInstructions_render(opts)
    local tools = dai.detectToolCapabilities(opts.tools)
    local lines = {}
    table.insert(
        lines,
...
--- Resolve for Hidden Model B.
--- Note: The original resolver uses isHiddenModelB (hash-based) which cannot
--- be detected by model name prefix in Lua. The capabilities function returns
--- false in Lua since hashes aren't ported. This prompt should be used via
--- explicit caller delegation or registry override.
---@param opts Copilot.Options
...
function M.resolve(_)
    return M.HiddenModelBPrompt_render, M.HiddenModelBReminderInstructions_render
end
