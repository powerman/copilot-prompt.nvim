--- Family H model-specific prompts.
--- Ported from familyHPrompts.tsx.
...
--- DefaultFamilyHAgentPrompt — for Family H models.
---@param opts Copilot.Options
---@return string
function M.DefaultFamilyHAgentPrompt_render(opts)
    ...
    if tools.CoreRunInTerminal then
        ...
        table.insert(
            lines,
            '- `'
                .. tn(tools, 'CoreGetTerminalOutput')
                .. '` — get output from a backgrounded command'
        )
        table.insert(
            lines,
            '- `'
                .. tn(tools, 'CoreTerminalLastCommand')
                .. '` — get the last command run in a terminal'
        )
        table.insert(
            lines,
            '- `'
                .. tn(tools, 'CoreTerminalSelection')
    ...
    table.insert(
        ...
        tag(
            ...
            table.concat({
                ...
                '- When referencing workspace files or lines, use backtick formatting',
                ...
                '',
                '_Filepath comments_ — when showing code that belongs to a specific workspace file, include a filepath comment as the very first line of the block. This enables code application actions:',
                '',
                '````typescript',
                ...
                '````',
                '',
                'Use `#` for Python/shell, `//` for JS/TS/C-style, `--` for SQL, etc.',
                '',
                ...
                tag(
                    'good-example',
                    table.concat({
                        'GOOD - Partial edit with filepath and existing code markers:',
                        '````python',
...
--- FamilyHReminderInstructions — falls back to DefaultReminderInstructions.
---@param opts Copilot.Options
---@return string
function M.FamilyHReminderInstructions_render(opts)
    return dai.DefaultReminderInstructions_render(opts)
end

--- Resolve which Family H prompt to use.
---@param opts Copilot.Options
...
function M.resolve(_)
    return M.DefaultFamilyHAgentPrompt_render, M.FamilyHReminderInstructions_render
end
