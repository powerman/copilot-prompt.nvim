    ...
    do
        ...
        if tools.CoreRunInTerminal then
            local terminalTool = tools.ExecutionSubagent
                    and (tn(tools, 'ExecutionSubagent') .. ' or ' .. tn(
                        tools,
                        'CoreRunInTerminal'
                    ))
...
--- Render the optimized base for Claude 4.6+ models (Sonnet/Opus).
--- Two variants differ in <instructions> exploration guidance and <parallelizationStrategy>.
---@param opts Copilot.Options
---@param explorationGuidance string Lines (already joined with newline) inserted into <instructions>.
---@param parallelizationStrategy string Body of the <parallelizationStrategy> tag.
---@return string
local function renderClaude46Optimized(opts, explorationGuidance, parallelizationStrategy)
    local tools = dai.detectToolCapabilities(opts.tools)
    local contextCompactionEnabled = opts.anthropicContextEditingEnabled
    local parts = {}
    ...
    do
        ...
        if explorationGuidance and explorationGuidance ~= '' then
            table.insert(lines, explorationGuidance)
        end
        table.insert(
            lines,
    ...
    if parallelizationStrategy and parallelizationStrategy ~= '' then
...
--- Moderate exploration guidance balancing persistence with bounding.
---@param opts Copilot.Options
---@return string
function M.Claude46SonnetPrompt_render(opts)
    local exploration = table.concat({
    ...
    return renderClaude46Optimized(opts, exploration, parallelization)
end

--- Claude46OpusPrompt — for Claude 4.6+ Opus models.
--- Bounded exploration guidance to reduce over-exploration observed in benchmarks.
---@param opts Copilot.Options
---@return string
...
--- Claude46DefaultPrompt — backwards-compatible alias defaulting to Sonnet variant.
---@param opts Copilot.Options
---@return string
...
--- AnthropicReminderInstructions — for Sonnet4 and Claude45.
---@param opts Copilot.Options
---@return string
...
--- AnthropicReminderInstructionsOptimized — condensed variant for Claude 4.6+ models.
...
function M.AnthropicReminderInstructionsOptimized_render(opts)
    ...
    if tools.EditFile and tools.ReplaceString then
        local prefer = tn(tools, 'ReplaceString')
        if tools.MultiReplaceString then
            prefer = prefer .. ' or ' .. tn(tools, 'MultiReplaceString')
        end
        table.insert(lines, 'Prefer ' .. prefer .. ' over ' .. tn(tools, 'EditFile') .. '.')
    end
