...
--- Includes DefaultAnthropicAgentPrompt, Claude45DefaultPrompt, Claude46OptimizedBasePrompt, Claude46SonnetPrompt, Claude46OpusPrompt.
...
--- ToolSearchToolPromptOptimized — condensed variant for Claude 4.6+ optimized prompts.
--- Ported from ToolSearchToolPromptOptimized in anthropicPrompts.tsx.
---@param _ Copilot.Options
---@param tools table
---@return string
local function ToolSearchToolPromptOptimized_render(_, tools)
    if not tools.ToolSearch then
        return ''
    end
    local searchToolName = tn(tools, 'ToolSearch')
    return tag(
        ...
        table.concat({
            ...
            'Do NOT call '
                .. searchToolName
                .. ' again for a tool already returned by a previous search. If a search returns no matching tools, the tool is not available. Do not retry with different patterns.',
        }, '\n')
    )
end
...
--- Claude46OptimizedBasePrompt — shared base for Claude 4.6+ optimized prompts.
--- Not exported; used by Claude46SonnetPrompt and Claude46OpusPrompt.
---@param opts Copilot.Options
---@param explorationGuidance string
---@param parallelizationStrategy string
---@return string
local function Claude46OptimizedBasePrompt_render(
    opts,
    explorationGuidance,
    ...
    table.insert(
        ...
        tag(
            ...
            table.concat({
                ...
                "By default, implement changes rather than only suggesting them. If the user's intent is unclear, infer the most useful likely action and proceed with using tools to discover any missing details instead of guessing.",
                explorationGuidance,
    ...
    -- securityRequirements tag (condensed)
    table.insert(
        parts,
        tag(
    ...
    -- operationalSafety tag (condensed)
    table.insert(
        parts,
        tag(
            'operationalSafety',
    ...
    -- implementationDiscipline tag (condensed)
    table.insert(
        parts,
        tag(
    ...
    -- parallelizationStrategy tag
    table.insert(parts, tag('parallelizationStrategy', parallelizationStrategy))

    -- taskTracking tag (condensed)
    if tools.CoreManageTodoList then
        table.insert(
            parts,
            tag(
    ...
    -- contextManagement tag (condensed)
    if contextCompactionEnabled then
        table.insert(
            parts,
            tag(
    ...
    -- toolUseInstructions tag (condensed)
    do
        local lines = {}
        table.insert(
            lines,
        ...
        local parallelLine = 'Call independent tools in parallel'
        if tools.Codebase then
        ...
        table.insert(lines, parallelLine .. '. Call dependent tools sequentially.')
        if tools.CoreRunInTerminal then
            table.insert(
                lines,
                tn(tools, 'CoreRunInTerminal')
            ...
            table.insert(
                ...
                tn(tools, 'CoreRunInTerminal')
                    ...
                    .. tn(tools, 'ReadFile')
                    .. ', '
                    .. tn(tools, 'ListDirectory')
                    .. ') have been optimized specifically for this environment. Default to using these tools over lower level terminal commands when the operation can be performed by them.'
            )
        end
        if tools.SearchSubagent then
            table.insert(
        ...
        if tools.ToolSearch then
            table.insert(lines, ToolSearchToolPromptOptimized_render(opts, tools))
        end
        table.insert(parts, tag('toolUseInstructions', table.concat(lines, '\n')))
    end

    -- communicationStyle tag (condensed)
    table.insert(
        parts,
        tag(
            'communicationStyle',
    ...
    -- outputFormatting tag with optimized fileLinkification
    do
        local lines = {}
        table.insert(
            lines,
        ...
        table.insert(lines, fileLinkification.renderOptimized())
...
--- Claude46SonnetPrompt — for Claude 4.6+ Sonnet models.
---@param opts Copilot.Options
---@return string
function M.Claude46SonnetPrompt_render(opts)
    return Claude46OptimizedBasePrompt_render(
        opts,
        table.concat({
...
--- Claude46OpusPrompt — for Claude 4.6+ Opus models.
---@param opts Copilot.Options
---@return string
...
--- AnthropicReminderInstructionsOptimized — condensed reminder for Claude 4.6+ optimized prompts.
---@param opts Copilot.Options
---@return string
