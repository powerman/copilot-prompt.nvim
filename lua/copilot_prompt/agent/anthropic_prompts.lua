...
--- Claude46OptimizedBasePrompt — base for Claude 4.6+ models with optimized content.
--- Sub-functions provide exploration guidance and parallelization strategy.
---@param opts Copilot.Options
---@return string
local function Claude46OptimizedBase_render(opts, explorationGuidance, parallelizationStrategy)
    local tools = dai.detectToolCapabilities(opts.tools)
    local contextCompactionEnabled = opts.anthropicContextEditingEnabled
    local parts = {}
    ...
    if explorationGuidance then
        table.insert(instrLines, explorationGuidance)
    end
    table.insert(
        instrLines,
    ...
    table.insert(instrLines, 'Avoid giving time estimates.')
    table.insert(parts, tag('instructions', table.concat(instrLines, '\n')))

    -- securityRequirements (condensed)
    table.insert(
        parts,
        tag(
    ...
    -- implementationDiscipline (condensed)
    table.insert(
        parts,
        tag(
    ...
    if parallelizationStrategy then
        table.insert(parts, parallelizationStrategy)
    end

    -- taskTracking (condensed)
    if tools.CoreManageTodoList then
        table.insert(
            parts,
            tag(
    ...
    do
        ...
        parallel = parallel .. '. Call dependent tools sequentially.'
        table.insert(lines, parallel)
        if tools.CoreRunInTerminal then
            table.insert(
                lines,
...
--- Claude46SonnetPrompt — moderate exploration guidance for Sonnet 4.6.
---@param opts Copilot.Options
---@return string
function M.Claude46SonnetPrompt_render(opts)
    ...
    local parallelizationStrategy = tag(
    ...
    return Claude46OptimizedBase_render(opts, explorationGuidance, parallelizationStrategy)
end

--- Claude46OpusPrompt — bounded exploration guidance for Opus 4.6.
---@param opts Copilot.Options
---@return string
...
--- AnthropicReminderInstructionsOptimized — condensed reminder for Claude 4.6+.
---@param opts Copilot.Options
---@return string
local function AnthropicReminderInstructionsOptimized_render(opts)
    local tools = dai.detectToolCapabilities(opts.tools)
    local result = ''
    ...
    if tools.EditFile and tools.ReplaceString then
        local prefer = 'Prefer ' .. tn(tools, 'ReplaceString')
        if tools.MultiReplaceString then
            prefer = prefer .. ' or ' .. tn(tools, 'MultiReplaceString')
        end
        prefer = prefer .. ' over ' .. tn(tools, 'EditFile') .. '.\n'
        result = result .. prefer
    end
...
function M.resolve(opts)
    ...
    local systemPrompt
    local reminder = M.AnthropicReminderInstructions_render
    if isSonnet4 then
    ...
    elseif isOpus then
        ...
        reminder = AnthropicReminderInstructionsOptimized_render
    else
