...
function M.DefaultAnthropicAgentPrompt_render(opts)
    ...
    do
        ...
        if tools.CoreRunInTerminal then
            ...
            if tools.ExecutionSubagent then
                codeblock_msg = codeblock_msg .. tn(tools, 'ExecutionSubagent') .. ' or '
            end
            codeblock_msg = codeblock_msg .. tn(tools, 'CoreRunInTerminal') .. ' tool instead.'
            table.insert(lines, codeblock_msg)
        end
        table.insert(
            lines,
...
--- Shared base renderer for optimized Claude 4.6 prompts.
--- @param opts Copilot.Options
--- @param exploration_guidance string|nil additional lines for instructions tag
--- @param parallelization_strategy string|nil content for parallelizationStrategy tag
--- @return string
local function claude46_base_render(opts, exploration_guidance, parallelization_strategy)
    local tools = dai.detectToolCapabilities(opts.tools)
    local contextCompactionEnabled = opts.anthropicContextEditingEnabled
    local parts = {}
    ...
    do
        ...
        if exploration_guidance then
            table.insert(lines, exploration_guidance)
        end
        table.insert(
            lines,
        ...
        table.insert(lines, 'Avoid giving time estimates.')
        table.insert(parts, tag('instructions', table.concat(lines, '\n')))
    end
    ...
    if parallelization_strategy then
        table.insert(parts, tag('parallelizationStrategy', parallelization_strategy))
    end
    ...
    table.insert(
        ...
        tag(
            ...
            table.concat({
                ...
                fileLinkification.renderOptimized(),
...
--- Claude46SonnetPrompt — condensed prompt for Claude 4.6 Sonnet models.
--- Ported from Claude46SonnetPrompt in anthropicPrompts.tsx.
---@param opts Copilot.Options
---@return string
function M.Claude46SonnetPrompt_render(opts)
    local exploration_guidance = table.concat({
    ...
    local parallelization_strategy =
    ...
    return claude46_base_render(opts, exploration_guidance, parallelization_strategy)
end

--- Claude46OpusPrompt — condensed prompt for Claude 4.6 Opus models.
--- Ported from Claude46OpusPrompt in anthropicPrompts.tsx.
--- Uses bounded exploration guidance to reduce over-exploration.
---@param opts Copilot.Options
---@return string
...
--- AnthropicReminderInstructionsOptimized — condensed reminder for Claude 4.6+ models.
--- Ported from AnthropicReminderInstructionsOptimized in anthropicPrompts.tsx.
--- Inlines editing reminder and removes the ToolSearch reminder block.
---@param opts Copilot.Options
---@return string
function M.AnthropicReminderInstructionsOptimized_render(opts)
    ...
    result = result .. 'Do NOT create markdown files to document changes unless requested.\n'
    if opts.anthropicContextEditingEnabled then
        result = result
...
function M.resolve(opts)
    ...
    if isSonnet4 or isClaude45 then
        reminderFn = M.AnthropicReminderInstructions_render
    else
        reminderFn = M.AnthropicReminderInstructionsOptimized_render
    end

    return systemPrompt, reminderFn
end
