    ...
    return tag(
        ...
        table.concat({
            ...
            tag(
                ...
                'MCP servers may add or remove tools dynamically during a conversation via tools/list_changed notifications. If you called a tool that may have enabled new tools on an MCP server, search for the new tools — they may now be discoverable even if not listed in the available deferred tools list above.'
            ),
            '',
            tag(
                'availableDeferredTools',
                'List of tools available in your workspace (deferred — must be loaded before use):'
            ),
            '',
            ...
            '',
            'Prefer broad queries that cover all related tools in a single search. For example, search "github" to find all GitHub tools at once rather than making separate searches for issues and pull requests. Check the available deferred tools list above and use it to inform your query.',
        }, '\n')
    )
end
        ...
        if tools.CoreRunInTerminal then
            table.insert(
                ...
                'NEVER print out a codeblock with a terminal command to run unless the user asked for it. Use the '
                    .. (tools.ExecutionSubagent and (tn(tools, 'ExecutionSubagent') .. ' or ') or '')
                    .. tn(tools, 'CoreRunInTerminal')
                    .. ' tool instead.'
            )
        end
...
--- Optimized base for Claude 4.6+ prompts.
--- Renders shared sections; exploration guidance and parallelization strategy
--- are provided by model-specific subclasses.
---@param opts Copilot.Options
---@param explorationGuidance string|nil
---@param parallelizationStrategy string|nil
---@return string
function M.Claude46OptimizedBasePrompt_render(
    opts,
    explorationGuidance,
        ...
        tag(
            ...
            table.concat(
                vim.tbl_filter(function(x)
                    ...
                    explorationGuidance and explorationGuidance or nil,
    ...
    if parallelizationStrategy then
    ...
    -- toolUseInstructions — optimized for Claude 4.6
    do
        local lines = {}
        table.insert(
            lines,
        ...
        local parallelBase = 'Call independent tools in parallel'
        if tools.Codebase then
            table.insert(
                lines,
                parallelBase
                    .. ', but do not call '
                    .. tn(tools, 'Codebase')
                    .. ' in parallel. Call dependent tools sequentially.'
            )
        else
            table.insert(lines, parallelBase .. '. Call dependent tools sequentially.')
        end
        if tools.CoreRunInTerminal then
            table.insert(
                lines,
    ...
    table.insert(
        ...
        tag(
            ...
            table.concat({
                ...
                dai.outputFormattingTagWithLinks(opts),
            }, '\n')
        )
    )
...
--- Claude46SonnetPrompt — optimized prompt for Sonnet 4.6.
---@param opts Copilot.Options
---@return string
function M.Claude46SonnetPrompt_render(opts)
    local explorationGuidance = table.concat({
    ...
    local parallelizationStrategy = table.concat({
    ...
    return M.Claude46OptimizedBasePrompt_render(
        opts,
        explorationGuidance,
        parallelizationStrategy
    )
...
--- Claude46OpusPrompt — optimized prompt for Opus 4.6.
---@param opts Copilot.Options
---@return string
...
--- Keep the old Claude46DefaultPrompt as fallback for non-Sonnet/Opus Claude 4.6+ models.
---@param opts Copilot.Options
---@return string
function M.Claude46DefaultPrompt_render(opts)
    return M.Claude46SonnetPrompt_render(opts)
end
...
--- AnthropicReminderInstructionsOptimized — condensed reminder for Claude 4.6 Sonnet/Opus.
---@param opts Copilot.Options
---@return string
function M.AnthropicReminderInstructionsOptimized_render(opts)
    ...
    if tools.EditFile then
        table.insert(
            ...
            'When using '
                .. tn(tools, 'EditFile')
                .. ', use line comments to represent unchanged regions.'
        )
    end
    ...
    if tools.EditFile and tools.ReplaceString then
        local replaceRef = tn(tools, 'ReplaceString')
        if tools.MultiReplaceString then
            replaceRef = replaceRef .. ' or ' .. tn(tools, 'MultiReplaceString')
        end
        table.insert(lines, 'Prefer ' .. replaceRef .. ' over ' .. tn(tools, 'EditFile') .. '.')
    end
