...
--- Claude46OptimizedBasePrompt — shared optimized base for Claude 4.6+ models.
---@param opts Copilot.Options
...
local function Claude46OptimizedBasePrompt_render(
    ...
    table.insert(
        ...
        tag(
            ...
            table.concat({
                ...
                table.concat(explorationGuidance, '\n'),
    ...
    if #parallelizationStrategy > 0 then
        table.insert(
            parts,
            tag('parallelizationStrategy', table.concat(parallelizationStrategy, '\n'))
        )
    end
    ...
    do
        ...
        if tools.CoreRunInTerminal then
            local customTools = {}
            for _, t in ipairs { 'FindTextInFiles', 'FindFiles', 'ReadFile', 'ListDirectory' } do
                if tools[t] then
                    table.insert(customTools, tn(tools, t))
                end
            end
    ...
    do
        ...
        table.insert(
            lines,
            require('copilot_prompt.agent.file_linkification_instructions').renderOptimized()
        )
...
--- Claude46SonnetPrompt — optimized for Sonnet 4.6.
---@param opts Copilot.Options
---@return string
function M.Claude46SonnetPrompt_render(opts)
    return Claude46OptimizedBasePrompt_render(opts, {
...
--- Claude46OpusPrompt — optimized for Opus 4.6.
---@param opts Copilot.Options
---@return string
...
--- AnthropicReminderInstructionsOptimized — for Claude 4.6+.
---@param opts Copilot.Options
---@return string
...
function M.resolve(opts)
    ...
    local systemPrompt
    local reminderInstructions = M.AnthropicReminderInstructions_render
    if isSonnet4 then
    ...
    elseif isOpus then
        ...
        reminderInstructions = M.AnthropicReminderInstructionsOptimized_render
    else
        systemPrompt = M.Claude46SonnetPrompt_render
        reminderInstructions = M.AnthropicReminderInstructionsOptimized_render
    end

    return systemPrompt, reminderInstructions
end
